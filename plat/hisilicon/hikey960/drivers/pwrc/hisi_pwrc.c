...
#define CLUSTER_IDLE_MASK \
			(CLUSTER_IDLE_BIT | 0x0F)
...
static void hisi_resource_lock(unsigned int lockid, unsigned int offset)
{
	...
	do {
		writel(lock_val, offset);
		lock_state = readl((LOCK_STAT_OFFSET + (uintptr_t)offset));
...
static void hisi_resource_unlock(unsigned int lockid, unsigned int offset)
{
	...
	writel(lock_val, (LOCK_UNLOCK_OFFSET + (uintptr_t)offset));
}
...
unsigned int hisi_get_cpuidle_flag(unsigned int cluster)
{
	...
	val = readl(CPUIDLE_FLAG_REG(cluster));
	val &= 0xF;
...
void hisi_set_cpuidle_flag(unsigned int cluster, unsigned int core)
{
	...
	val = readl(CPUIDLE_FLAG_REG(cluster));
	val |= BIT(core);
	writel(val, CPUIDLE_FLAG_REG(cluster));
}
				...
				unsigned int core, unsigned int value)
{
	...
	val = readl(REG_SCBAKDATA3_OFFSET);
	val = (value << (cluster << 1)) | (val & 0xFFFFFFF);
	writel(val, REG_SCBAKDATA3_OFFSET);
...
{
	unsigned int val = readl(REG_SCBAKDATA3_OFFSET);
	int ret;
...
static void *hisi_get_pdc_addr(unsigned int cluster)
{
	...
	if (cluster == 0)
		pdc_base_addr = (void *)(SOC_CRGPERIPH_A53_PDCEN_ADDR(CRG_BASE));
	else
		pdc_base_addr = (void *)(SOC_CRGPERIPH_MAIA_PDCEN_ADDR(CRG_BASE));
...
static unsigned int hisi_get_pdc_stat(unsigned int cluster)
{
	...
	val = readl(((uintptr_t)pdc_base_addr + PDC_COREPOWERSTAT_OFFSET));
...
{
	unsigned int val = readl(((uintptr_t)pdc_base_addr + PDC_COREPWRINTEN_OFFSET));
	if (intmask == PDC_ENABLE_FINISH_INT)
		val |= BIT(core);
	else
		val &= ~BIT(core);

	writel(val, ((uintptr_t)pdc_base_addr + PDC_COREPWRINTEN_OFFSET));
}

static inline void hisi_pdc_set_gicmask(void *pdc_base_addr,
				unsigned int core, enum pdc_gic_mask gicmask)
{
	unsigned int val = readl(((uintptr_t)pdc_base_addr + PDC_COREGICMASK_OFFSET));
	if (gicmask == PDC_MASK_GIC_WAKE_IRQ)
		val |= BIT(core);
	else
		val &= ~BIT(core);

	writel(val, ((uintptr_t)pdc_base_addr + PDC_COREGICMASK_OFFSET));
}
		...
		enum pdc_finish_int_mask intmask)
{
	...
	writel(BIT(core), ((uintptr_t)pdc_base_addr + PDC_COREPOWERUP_OFFSET));
}

static void hisi_pdc_powerdn_core(unsigned int cluster, unsigned int core,
		enum pdc_gic_mask gicmask, enum pdc_finish_int_mask intmask)
{
	void *pdc_base_addr = hisi_get_pdc_addr(cluster);

	writel(BIT(core), ((uintptr_t)pdc_base_addr + PDC_COREPOWERDN_OFFSET));
}

void hisi_powerup_core(unsigned int cluster, unsigned int core)
{
	hisi_pdc_powerup_core(cluster, core,
					PDC_MASK_GIC_WAKE_IRQ,
					PDC_DISABLE_FINISH_INT);
}
...
{
	hisi_pdc_powerdn_core(cluster, core,
					PDC_MASK_GIC_WAKE_IRQ,
					PDC_DISABLE_FINISH_INT);
}
...
void hisi_powerdn_cluster(unsigned int cluster, unsigned int core)
{
	...
	writel((0x10001 << core), ((uintptr_t)pdc_base_addr + PDC_COREPWRINTEN_OFFSET));
	writel(BIT(core), ((uintptr_t)pdc_base_addr + PDC_COREPOWERDN_OFFSET));
}
